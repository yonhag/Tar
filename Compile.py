...
def remove_existing_containers():
    try:
        ...
        print('Docker down!\nActivate Docker to run this script!')
        exit()


def run_project():
    run_relay_command = 'docker run --memory=3g --cpus=2 -it -p 12345:12345 --network=tor --ip 192.168.0.5 relay 12345'
    run_dir_command = 'docker run --memory=3g --cpus=2 -it -p 12345:12345 --network=tor --ip 192.168.0.27 dir'
    ...
    for i in range(6, 9):
        time.sleep(0.5)
        subprocess.Popen(['start', 'cmd', '/k', run_relay_command.replace('5', str(i))], shell=True)
...
def transfer_dir_executable():
    try:
        ...
        print("Error in dir compilation")
        exit()
...
def transfer_relay_executable():
    try:
        ...
        print("Error in relay compilation")
        exit()
